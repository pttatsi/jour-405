...
**Write your answer here** Generally, 911 overdose calls in Baltimore county stay below 20 on a daily basis. The closeness of the mean and median indicates that there are little to no major outliers in our data set however the visualization show multiple outliers 
...
**Write your answer here** It validates my observation that there are outliers in the dataset despite the closeness of the mean (10.44) and median (10.00). The day of week summary shows also that the average number of calls was approximately 9-10 per day, and we can see the spike of 11 calls happens during the weekend. We can infer that this happens because people have more time to go out and socialize with substances more. 
...
On Oct. 4th 2022, the total call count was 23, which is an extremely unusual call volume for a Tuesday in Baltimore County. For comparison, the average call count on Tuesdays is about 10.35 calls. The same is true for July 14th 2022, which had 21 calls, a significant shift from its usual average of 9.4 calls. 
...
#ask in questions what this means/when to know to use it
daily_calls <- daily_calls |>
  arrange(date) |>
...
**Write your answer here** This dataset spans Feb. 2022 to Feb. 2023 so its a bit incomplete to get a full fill of overdoses in Baltimore county. So my next step would be to obtain information to complete the dataset and compare to other years to see if trends are consistent.  Looking at monthly_summary, there's no easy-to-see seasonality spikes but there could be if I got more data. Moreso, I could do more research and match the top dates with events in Baltimore county. 
** I would look more into Tuesday Oct. 4th 2022 because it is three standard deviations away from the mean, thus a significant outlier. I couldn't find much reported on that day for Baltimore County but it could be investigated further to find out why a Tuesday, in winter, saw a spike in overdose calls. To an editor, I would flag recent police activity targeting drug networks—such as raids in the Penn North area and note that these enforcement actions may connect to overdose trends.