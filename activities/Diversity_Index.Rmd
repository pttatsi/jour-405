...
**ANSWER HERE** Each row in our dataset represents a maryland city. Each column represents population and demographic information for each city. Based on this raw data, Baltimore city and Germantown stand out as diverse because they have the highest number of two plus races. 
...
**ANSWER HERE** Summerfield has the highest probability that two people randomly chosen from the same city, will be the same race because they have low diversity. Summerfield has a high black population but low numbers for other races. 

## Task 5: Calculate Ethnicity Percentages and Their Squares (3 points)

Hispanic origin is not a race but an ethnicity or nationality – and a separate Census question. The probability someone is Hispanic or not Hispanic must be calculated separately from race, and 

Hisp_Prob is the probability that two randomly selected people will be Hispanic or not. 

That's an important distinction, because unlike race probability, this measures the likelihood of a binary question: that both random people will be Hispanic. What do you notice about the cities with the *lowest* Hisp_Prob scores?
...
**ANSWER HERE** Chillium has the lowest Hisp_Prob score because the population is more Black than Hispanic so that weakens the probability that two randomly selected people will be Hispanic. 

## Task 6: Multiply Race and Ethnicity Probabilities

Here we multiply the two probabilities together to get 

the probability that any two random people have the SAME race and ethnicity.
...
**ANSWER HERE**  Langely Park, Wheaton and Adelphi are the most diverse cities according to the index scores. The least diverse cities are Lake Shore, Lake Arbor and Largo, according to the index scores. 
...
**ANSWER HERE** 
HL: Maryland’s least diverse cities are predominantly Black, USA Today Diversity Index finds

Lede: Black people are the top demogrpahic for Maryland's least diverse cities such as Lake Arbor, Lake Shore and Largo, according to the USA Today Diversity Index. In these cities, residents are more likely to encounter others with the same race and ethnicity as them.

How would I report this?: 
- Compare median income, homeownership rates, and education levels across these cities
- Residents: Ask longtime and newer residents why they chose (or stayed in) these areas, what makes the community appealing or limiting.
- Speak to city officials to get insight into housing policies and economic development (Largo is a transit town with new apartments so I am surprised that they have low diversity, but this could also be a price related barrier)
- History of segregation/migration in the area
- Is the low diversity driven by affordability, historic patterns of segregation, or community preference?
- also PG county is predominantly Black and African
