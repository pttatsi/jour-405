...
**Yes it is a very useful model because you can compare one dependent variable against multiple independent variables which can save time and open room for more questions and thus deeper analysis
...
ANSWER HERE ** Points scored, with a p-value of 0.006 has the most impact on wins. The least impactful seems to be yards gained but a closer look at its correlation to wins shows that yards gained has a strong positive relationship to points scored, meaning a rise in it led to a rise in points scored. But its not statistically significant on its own in securing wins, you have to score a point. 

## Using the Model

Without using the words "multiple regression model", describe how you could use this model to cover a team or game.
-- Though the Steelers gained more yards during the game, they were unable to secure enough wins to become the Superbowl champions.