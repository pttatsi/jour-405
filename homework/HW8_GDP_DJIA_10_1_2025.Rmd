...
```{r}
data_cor <- read_csv("https://raw.githubusercontent.com/dwillis/jour405/refs/heads/main/data/gdp_djia.csv")
```
...
```{r}
data_cor |> summarize(correlation = cor(GDP, DJIA, method="pearson"))
```
...
labs(title = "Relationship Between DJIA and GDP", x = "GDP", y = "DJIA")
```
...
The scatterplot reveals a strong relationship between the DJIA and GDP, they are positively related as they move in tandem throughout the years. It can also be assumed or theorized that both variables are affected by the same factors such as inflation. 
